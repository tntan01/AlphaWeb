...
export default function AddProduct() {
  ...
  const [brands, setBrands] = useState<any[]>([]) // State danh sách hãng
  ...
  const [brandId, setBrandId] = useState('') // Lưu brand_id được chọn
  ...
  // Nạp Chuyên mục và Hãng sản xuất (Sắp xếp ABC)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('products').insert([{
      ...
      brand_id: brandId || null, // Thêm hãng sản xuất
      status,
    ...
    if (!error) {
      ...
      router.push('/admin/products') // Link chuẩn về trang danh sách
        ...
        <div className="space-y-8">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Hãng sản xuất</label>
            ...
            <button disabled={loading} className="w-full py-5 bg-blue-600 text-white rounded-[1.5rem] font-black shadow-lg shadow-blue-100 hover:bg-blue-700 transition active:scale-95 disabled:bg-slate-200 uppercase tracking-widest">
              {loading ? 'Đang lưu...' : 'Đăng sản phẩm Alpha'}
            </button>
          </div>

          <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-50 space-y-6">
            <h3 className="font-bold text-slate-800 px-2 uppercase tracking-tighter">🖼️ Media Sản phẩm</h3>
            ...
            <div className="space-y-2 pt-4 border-t border-slate-50">
              <label className="text-[10px] font-black text-slate-400 uppercase">Gallery Ảnh ({gallery.length})</label>
